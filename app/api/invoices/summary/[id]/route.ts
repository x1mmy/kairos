...
const schema = z.object({ status: z.enum(["draft", "finalised"]) })
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const { error } = await auth.supabase.from("internal_summaries").update({ status: parsed.data.status }).eq("id", params.id)
