    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.headingRow}>
          <View>
            <Text style={styles.brand}>KAIROS</Text>
            <Text style={[styles.title, { fontSize: 30 }]}>INTERNAL SUMMARY</Text>
          </View>
          <View>
            <Text style={styles.metaLabel}>Summary Number</Text>
            <Text style={styles.metaValue}>{summaryNumber}</Text>
            <Text style={[styles.metaLabel, { marginTop: 8 }]}>Period</Text>
        ...
        <View style={styles.section}>
          <View style={styles.tableHeader}>
            <Text style={styles.descCol}>Payee</Text>
            <Text style={styles.unitCol}>Invoice #</Text>
            <Text style={styles.amountCol}>Amount</Text>
          </View>
          {payees.map((row, idx) => (
            <View key={idx} style={styles.tableRow}>
              <Text style={styles.descCol}>{row.name}</Text>
              <Text style={styles.unitCol}>{row.invoice_number}</Text>
              <Text style={styles.amountCol}>{formatCurrency(row.total_amount)}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.totalsWrap}>
          ...
          <View style={styles.totalLine}>
            ...
            <Text style={styles.totalValue}>{formatCurrency(budget)}</Text>
          </View>
          <View style={styles.grandTotalLine}>
            <Text style={styles.grandTotalLabel}>VARIANCE</Text>
            <Text style={styles.grandTotalValue}>{formatCurrency(variance)}</Text>
          </View>
        </View>

        <Text style={styles.footer}>Generated by Kairos internal invoicing and budget management.</Text>
      </Page>
    </Document>
  )
}
